  ...
  const SCOPE = "" // Add the scopes you need here      // Scopes to get specific data from API
    ...
    const renderResults = () => {
      return results.map(result => (
        <div key={result.id}>
        <br></br>
        {result.images.length ? <img width = "300px" height = "300px" src ={result.images[0].url} alt=""/> : <div>No Image</div>}
        <br></br>
        <h1>{result.name}</h1>
        <h3>Followers: {result.followers.total}</h3>
        <h3>Main Genre: {result.genres[0]}</h3>
        {result.external_urls ? <Button className="btn1" href={result.external_urls.spotify}>View Artist in Spotify</Button> : <div></div>}
        </div>
      ))
    }
